...
```{r how-many-degrees}
# Figure out the number of each degree per year and plot it
degrees_per_year <- summarise(group_by(h_all, degree, year), count = length(id))
degrees_per_year <- arrange(degrees_per_year, year)
```

```{r degrees-per-year, fig.width=8, fig.height=6}
ggplot(degrees_per_year, aes(x = year, y = count, color = degree)) +
  geom_point() +
  geom_line() +
  ylab("Number") +
  xlab("") +
  ...
  xlim(1950,2012) +
  ggtitle("Dissertations and Theses about History, 1950-2012")
```

I'll be back with more analyses. If you have any questions that you think the data can answer, let me know.
